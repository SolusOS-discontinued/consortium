...
static GMarkupParser consortium_theme_parser = {
                 ...
                 GError    **err)
{
  ...
  for (version = THEME_VERSION; (version > 0) && (text == NULL); version--)
    {
      ...
      /* Look for themes in CONSORTIUM_DATADIR */
      if (text == NULL)
        {
  ...
  context = g_markup_parse_context_new (&consortium_theme_parser,
                                        0, &info, NULL);
  ...
  if (error)
  ...
  else
    {
      g_set_error (err, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                   _("Theme file %s did not contain a root <consortium_theme> element"),
                   info.theme_file);
    }
