 ...
 * kick everything off and then close down Consortium when it's time to go.
 */

/**
 * \mainpage
 * Consortium - a boring window manager for the adult in you
 *
 * Many window managers are like Marshmallow Froot Loops; Consortium
 * is like Cheerios.
 *
...
/**
 * Handle on the main loop, so that we have an easy way of shutting Consortium
 * down.
 */
static GMainLoop *meta_main_loop = NULL;

/**
 * If set, Consortium will spawn an identical copy of itself immediately
 * before quitting.
 */
...
/**
 * Prints log messages. If Consortium was compiled with backtrace support,
...
/**
 * Prints the version notice. This is shown when Consortium is called
 ...
 * build this copy of Consortium. This is actually always called
 ...
 * copy of Consortium. This is actually always called
 * on startup, but it's all no-op unless we're in verbose mode
 * (see meta_set_verbose).
 */
static void
meta_print_self_identity (void)
{
  ...
  meta_verbose ("Consortium version %s running on %s\n",
    VERSION, buf);
...
/**
 * The set of possible options that can be set on Consortium's
 ...
 * arguments that Consortium understands in meta_args.
 *
 * The strange call signature has to be written like it is so
 * that g_option_context_parse() gets a chance to modify argc and
 * argv.
 *
 * \param argc  Pointer to the number of arguments Consortium was given
 * \param argv  Pointer to the array of arguments Consortium was given
                    ...
                    MetaArguments *meta_args)
{
  ...
  GOptionEntry options[] = {
    ...
    },
    {
      ...
      &my_args.replace_wm,
      N_("Replace the running window manager with Consortium"),
      NULL
    },
    {
  ...
  g_option_context_add_main_entries (ctx, options, "consortium");
  if (!g_option_context_parse (ctx, argc, argv, &error))
    {
      g_print ("consortium: %s\n", error->message);
      exit(1);
    }
...
/**
 * Selects which display Consortium should use. It first tries to use
 * display_name as the display. If display_name is NULL then
 * try to use the environment variable CONSORTIUM_DISPLAY. If that
...
meta_select_display (gchar *display_name)
{
  ...
  else if (g_getenv ("CONSORTIUM_DISPLAY"))
    envVar = g_strconcat ("DISPLAY=",
      g_getenv ("CONSORTIUM_DISPLAY"), NULL);
  ...
  if (sigterm_pipe_fds[1] >= 0)
    {
      if ( write (sigterm_pipe_fds[1], "", 1) == -1 )
        g_printerr ("consortium: write to sigterm_pipe failed.\n");
...
main (int argc, char **argv)
{
  ...
  if (g_getenv ("CONSORTIUM_VERBOSE"))
    meta_set_verbose (TRUE);
  if (g_getenv ("CONSORTIUM_DEBUG"))
  ...
  bindtextdomain (GETTEXT_PACKAGE, CONSORTIUM_LOCALEDIR);
  ...
  meta_set_syncing (meta_args.sync || (g_getenv ("CONSORTIUM_SYNC") != NULL));
  ...
  if (g_getenv ("CONSORTIUM_G_FATAL_WARNINGS") != NULL)
  ...
  if (!meta_ui_have_a_theme ())
    {
      ...
      if (!(themes_dir = g_dir_open (CONSORTIUM_DATADIR"/themes", 0, &err)))
        {
...
/**
 * Stops Consortium. This tells the event loop to stop processing; it is rather
...
/**
 * Restarts Consortium. In practice, this tells the event loop to stop
 * processing, having first set the meta_restart_after_quit flag which
 * tells Consortium to spawn an identical copy of itself before quitting.
 * This happens on receipt of a _CONSORTIUM_RESTART_MESSAGE client event.
 */
void
meta_restart (void)
{
