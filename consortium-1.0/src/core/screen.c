...
reload_xinerama_infos (MetaScreen *screen)
{
...
#else
  meta_topic (META_DEBUG_XINERAMA,
              "Consortium compiled without XFree86 Xinerama support\n");
...
#else
  meta_topic (META_DEBUG_XINERAMA,
              "Consortium compiled without Solaris Xinerama support\n");
  ...
  if (screen->n_xinerama_infos == 0)
    {
      if (g_getenv ("CONSORTIUM_DEBUG_XINERAMA"))
        {
          meta_topic (META_DEBUG_XINERAMA,
...
set_desktop_viewport_hint (MetaScreen *screen)
{
  ...
  /*
   * Consortium does not implement viewports, so this is a fixed 0,0
   */
  data[0] = 0;
  data[1] = 0;
