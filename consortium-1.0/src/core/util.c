...
ensure_logfile (void)
{
  if (logfile == NULL && g_getenv ("CONSORTIUM_USE_LOGFILE"))
    {
      char *filename = NULL;
      char *tmpl;
      int fd;
      ...
      tmpl = g_strdup_printf ("consortium-%d-debug-log-XXXXXX",
                              (int) getpid ());
...
meta_set_verbose (gboolean setting)
{
  ...
  if (setting)
    meta_fatal (_("Consortium was compiled without support for verbose mode\n"));
#else 
  if (setting)
    ensure_logfile ();
#endif
                  ...
                  GSList *entries)
{
  ...
  argvl[i++] = "--class";
  argvl[i++] = "consortium-dialog";
  argvl[i++] = "--title";
  /* Translators: This is the title used on dialog boxes */
  argvl[i++] = _("Consortium");
  argvl[i++] = "--text";
  argvl[i++] = message;
