/* Consortium send-magic-messages app */
...
send_reload_theme (void)
{
  ...
  xev.xclient.window = gdk_x11_get_default_root_xwindow ();
  xev.xclient.message_type = XInternAtom (GDK_DISPLAY_XDISPLAY (gdk_display_get_default ()),
                                          "_CONSORTIUM_RELOAD_THEME_MESSAGE",
                                          False);
  xev.xclient.format = 32;
  xev.xclient.data.l[0] = 0;
  xev.xclient.data.l[1] = 0;
...
send_set_keybindings (gboolean enabled)
{
  ...
  xev.xclient.window = gdk_x11_get_default_root_xwindow ();
  xev.xclient.message_type = XInternAtom (GDK_DISPLAY_XDISPLAY (gdk_display_get_default ()),
                                          "_CONSORTIUM_SET_KEYBINDINGS_MESSAGE",
                                          False);
  xev.xclient.format = 32;
...
usage (void)
{
  g_printerr (_("Usage: %s\n"),
              "consortium-message (restart|reload-theme|enable-keybindings|disable-keybindings|toggle-verbose)");
  exit (1);
}
...
main (int argc, char **argv)
{
  ...
  if (strcmp (argv[1], "restart") == 0)
  ...
  else if (strcmp (argv[1], "toggle-verbose") == 0)
    {
      ...
      g_printerr (_("Consortium was compiled without support for verbose mode\n"));
      return 1;
#else      
      send_toggle_verbose ();
#endif
