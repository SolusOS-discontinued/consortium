...
#undef item
  ...
  the_display->static_gravity_works = g_getenv ("CONSORTIUM_USE_STATIC_GRAVITY") != NULL;
...
#undef item
   ...
   */
  {
    ...
    meta_prop_set_utf8_string_hint (the_display,
                                    the_display->leader_window,
                                    the_display->atom__CONSORTIUM_VERSION,
                                    VERSION);
 ...
 * it is the nexus, it is the Grand Central Station of Consortium's world.
    ...
    case ButtonPress:
      if ((window &&
      ...
      else if (window && display->grab_op == META_GRAB_OP_NONE)
        {
          ...
          if (g_getenv ("CONSORTIUM_DEBUG_BUTTON_GRABS"))
    ...
    case PropertyNotify:
      {
	     ...
	     */
	    if (event->xproperty.atom ==
		display->atom__CONSORTIUM_SENTINEL)
	      {
                  ...
                  if (showing_desktop)
                  ...
                  else
                    {
                      ...
                      meta_workspace_focus_default_window (screen->active_workspace, NULL, timestamp);
                       ...
                       display->atom__CONSORTIUM_RESTART_MESSAGE)
                {
                  meta_verbose ("Received restart request\n");
                  meta_restart ();
                }
              else if (event->xclient.message_type ==
                       display->atom__CONSORTIUM_RELOAD_THEME_MESSAGE)
                {
                  ...
                  meta_display_retheme_all ();
                       ...
                       display->atom__CONSORTIUM_SET_KEYBINDINGS_MESSAGE)
                {
                  ...
                  meta_set_keybindings_disabled (display, !event->xclient.data.l[0]);
                       ...
                       display->atom__CONSORTIUM_TOGGLE_VERBOSE)
                {
...
{  
  ...
  if (display->window_grab_modifiers != 0)
    {
      gboolean debug = g_getenv ("CONSORTIUM_DEBUG_BUTTON_GRABS") != NULL;
      int i;
      for (i = 1; i < 4; i++)
        {
                                     ...
                                     Window       xwindow)
{
  ...
  debug = g_getenv ("CONSORTIUM_DEBUG_BUTTON_GRABS") != NULL;
  i = 1;
  while (i < 4)
    {
      meta_change_button_grab (display, xwindow,
 ...
 * have any way to do so while Consortium is running, so it's rather
 * pointless.
 */
gboolean
