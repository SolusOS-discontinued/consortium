              ...
              int         state)
{
  ...
  /* Consortium doesn't use icon windows, so data[1] should be None
 ...
 * is useful for Consortium, but Consortium shouldn't need to specify a client
 * type for itself.  ;-)
 */
void
                                  ...
                                  int                  h)
{
   ...
   * are moved by consortium without resizing; send a configure notify
   * in such cases.  See #322840.  (Note that window->constructing is
   * only true iff this call is due to a MapRequest, and when
   * PPosition/UPosition hints aren't set, consortium seems to send a
                          ...
                          XEvent     *event)
{
   ...
   * and consortium's "pretend" focus window, and go through all
...
#define MAX_N_ACTIONS 12
  ...
  /* sticky according to EWMH is different from consortium's sticky;
   * consortium doesn't support EWMH sticky
   */
