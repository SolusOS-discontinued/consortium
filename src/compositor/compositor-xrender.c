...
{
  MetaScreen *screen;
  MetaWindow *window; /* May be NULL if this window isn't managed by Consortium */
  Window id;
...
window_has_shadow (MetaCompWindow *cw)
{
  ...
  {
        meta_verbose ("Window has no shadow as it is DnD\n");
        return FALSE;
  }

  if (cw->type == META_COMP_WINDOW_DOCK && cw->mode != WINDOW_ARGB)
  {
        meta_verbose ("Window has shadow as it is a non-ARGB dock");
        return TRUE;
  }

  if (cw->type == META_COMP_WINDOW_MENU || cw->type == META_COMP_WINDOW_DROP_DOWN_MENU
      || cw->type == META_COMP_WINDOW_POPUP_MENU)
  {
                 ...
                 MetaCompWindow *cw)
{
  ...
  for (i = 0; i < n_atoms; i++) 
    {
      if (atoms[i] == compositor->atom_net_wm_window_type_dnd ||
          ...
          atoms[i] == compositor->atom_net_wm_window_type_popup_menu)
        {
          type_atom = atoms[i];
          break;
        }
  ...
  else if (type_atom == compositor->atom_net_wm_window_type_popup_menu)
    cw->type = META_COMP_WINDOW_POPUP_MENU;
  else
         ...
         Window     xwindow)
{
  ...
  /* If Consortium has decided not to manage this window then the input events
                ...
                MetaWindow            *window)
{
  ...
  /* We are only interested in top level windows, others will
     be caught by normal consortium functions */
...
{
  compositor->show_redraw = (g_getenv ("CONSORTIUM_DEBUG_REDRAWS") != NULL);
  compositor->debug = (g_getenv ("CONSORTIUM_DEBUG_COMPOSITOR") != NULL);
                       ...
                       MetaWindow     *window)
{
   ...
   * enough about Consortium/X to know how else you are supposed to do it
   */
  meta_error_trap_push (xrc->display);
  switch (event->type) 
    {
...
meta_compositor_xrender_new (MetaDisplay *display)
{
  ...
  xrc->atom_net_wm_window_type_popup_menu = atoms[15];
